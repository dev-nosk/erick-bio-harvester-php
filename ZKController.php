...
if(!isset($_POST['ip'])){
    echo json_encode(['success' => 1, 'message' => 'IP address is required']);
    exit;
}
else {
    class ZKController {
    ...
    function pingIP($ip, $timeout = 1) {
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            $output = [];
            exec("ping -n 1 -w " . ($timeout * 1000) . " $ip", $output, $status);
            return $status === 0;
        } else {
            $output = [];
            exec("ping -c 1 -W $timeout $ip", $output, $status);
            return $status === 0;
        }
    }
    ...
    $ip = $_POST['ip'];
    if (!pingIP($ip)) {
        echo json_encode(['success' => 0, 'message' => 'Device is not reachable']);
        exit;
    }

    $zk = new ZKController($ip, 4370);
    $ret = $zk->getUser();
    var_dump('<pre>', $ret);
}
?>
